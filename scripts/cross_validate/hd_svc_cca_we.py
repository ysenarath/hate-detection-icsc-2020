...
from tklearn.text.word_vec import load_embedding
...
def run(dataset, features, word_vectors, metrics, fname):
    ...
    if 'mean_embedding' in features:
        ff += [('mean_embedding', mean_embedding(word_vectors), 'tokens')]
    # hatebase_vectorizer
    ...
    if 'transfer_vectorizer' in features:
        ...
        hyper_params['word_vectors'] = word_vectors
    ...
    for dataset in datasets:
        ...
        for fid0, feature_map in enumerate(features):
            for fid1, args in enumerate(feature_map[1]):
                ...
                run(dataset=dataset, features=feature_dict, word_vectors=hate_vectors, metrics=metrics,
